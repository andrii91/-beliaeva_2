$(document).ready(function () {
  ...
  var note = $('#note'),
//    ts = new Date(dateEnd),
     ts = new Date(2018, 06, 06, 23, 59),//new Date(year, month, date, hours, minutes, seconds, ms)
    newYear = true;
...
/*  if ((new Date()) > ts) {
  ...
  $('.countDays').append('<span class="timer_title">дней</span>');
  $('.countHours').append('<span class="timer_title">часов</span>');
  $('.countMinutes').append('<span class="timer_title">минут</span>');
  $('.countSeconds').append('<span class="timer_title">секунд</span>');
  ...
  $('.submit').click(function (e) {
    ...
    $(ref).each(function () {
      if ($(this).attr("type") == 'tel') {
        ...
        $(this).removeClass('error').parent('span').find('.allert').remove();
      }
